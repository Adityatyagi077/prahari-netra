...
export default defineConfig({
  ...
  server: {
    ...
    fs: { strict: true },
    proxy: {
      '/ai-api': { target: 'http://127.0.0.1:8000', changeOrigin: true, rewrite: (p) => p.replace(/^\/ai-api/, '') },
      '/signal-api': { target: 'http://127.0.0.1:3000', changeOrigin: true, rewrite: (p) => p.replace(/^\/signal-api/, '') },
    },
  },
